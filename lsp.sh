...
function mac() {
    ...
    if [[ ! (-z $output) ]]; then
        ...
        install)
            ...
            brew install llvm gopls marksman lua-language-server typescript-language-server python-lsp-server
...
function linux() {
    ...
    if [[ ! (-z $output) ]]; then
        ...
        case "$1" in
        install)
            sudo pacman -S python-lsp-server clang python-pip gopls lua-language-server tsserver zls --noconfirm
